...
def print_to_file(all_names, all_ticket_costs, all_surcharges):
    ...
    print(make_statement("Ticket Details", "-"))
    ...
    for i in range(total_tix_sold):
        ...
        print(f"{all_names[i]:<11} {to_currency(all_ticket_costs[i]):<14} {to_currency(all_surcharges[i]):<12} {to_currency(total_paid):<10} {to_currency(profit)}")
